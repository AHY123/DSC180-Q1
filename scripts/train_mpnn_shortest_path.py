"""Train MPNN (GIN/GCN/GAT) on shortest path prediction task."""
...
def main():
    ...
    cache_name = f'mpnn_sp_{args.num_graphs}graphs_k{args.k_pairs}.pt'
    dataset_config = {
        'name': dataset_name,
    ...
    # Node features already added by task: [degree, is_source, is_target]
    ...
    num_classes = args.max_distance + 1  # 0 to max_distance
    ...
    for epoch in range(args.epochs):
        ...
        # Early stopping
        if train_acc >= 0.95:
            print(f"\n✓ Model achieved 95% training accuracy (acc={train_acc*100:.2f}%)")
            print(f"Stopping early at epoch {epoch+1}")
            break

    # Save model
    os.makedirs("checkpoints", exist_ok=True)
    checkpoint_path = f"checkpoints/{args.model_type.lower()}_shortest_path_{args.num_graphs}graphs_k{args.k_pairs}.pt"
    torch.save({
