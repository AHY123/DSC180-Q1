...
class GraphTokenDataset(Dataset):
    ...
    def __init__(self, graphml_dir: str, vocab: Dict[str, int], max_graphs: int = None):
        ...
        if max_graphs:
            graphml_files = graphml_files[:max_graphs]
...
def build_vocab(graphml_dir: str, max_graphs: int = None) -> Dict[str, int]:
...
def main():
    ...
    # Build vocabulary
    vocab = build_vocab(DATA_DIR, max_graphs=args.num_graphs)

    # Create dataset
    dataset = GraphTokenDataset(DATA_DIR, vocab, max_graphs=args.num_graphs)
